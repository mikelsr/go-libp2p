...
import (
	...
	quiclogging "github.com/mikelsr/quic-go/logging"
	ma "github.com/multiformats/go-multiaddr"
	manet "github.com/multiformats/go-multiaddr/net"
)

// var quicDialContext = quic.DialContext // so we can mock it in tests
var quicDialContext = quic.Dial // so we can mock it in tests
...
func (c *ConnManager) DialQUIC(ctx context.Context, raddr ma.Multiaddr, tlsConf *tls.Config, allowWindowIncrease func(conn quic.Connection, delta uint64) bool) (quic.Connection, error) {
	...
	// TODO mikel removed the host string param
	conn, err := quicDialContext(ctx, pconn, naddr, tlsConf, quicConf)
	if err != nil {
		pconn.DecreaseCount()
		return nil, err
	}
